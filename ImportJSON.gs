...
function pullJSON() {
  ...
  var rowCount = SpreadsheetApp.getActiveSpreadsheet().getLastRow();
  var currentRow = 1;
  
  while (currentRow <= rowCount+6){
    var cell = sheet.getRange("C"+currentRow);
    var idCell = sheet.getRange("B"+currentRow);
    var id = idCell.getDisplayValue();
    var test = sheet.getRange("G"+currentRow);
    var testid = id.length;
    
    /*test.setFormula(testid);*/
    
    if (testid == 6){
      cell.setFormula('=ImportJSON("http://api.tradeskillmaster.com/v1/item/US/Dalaran/'+id+'?format=json&apiKey=AOM6vzxqJQfGrffVs8gaVKcSK778bZfB", "/MinBuyout", "noInherit,noTruncate,noHeaders,rawHeaders")');
      cell.copyTo(ss.getRange("Database!C"+currentRow), {contentsOnly: true});
      currentRow++;
    }
    else{
      currentRow++;
  ...
  while (currentRow <= 110){
    ...
    if (testid == 6){
      if(count < 51){
        ...
        cell.setFormula('=ImportJSON("http://api.tradeskillmaster.com/v1/item/US/Dalaran/'+id+'?format=json&apiKey=VRxOTGmueKp9y4QZphrdovt_rLiXMUNa", "/MinBuyout", "noInherit,noTruncate,noHeaders,rawHeaders")');
        cell.copyTo(ss.getRange("Database!C"+currentRow), {contentsOnly: true});
        currentRow++;
        count++;
      }
...
function test() {
  ...
  var cell1 = sheet.getRange("G2");
  var cell2 = sheet.getRange("G3");
  
  cell1.setFormula('=SUBSTITUTE(SUBSTITUTE(RIGHT(NOW(),8)," ",0),":","")');
  
  var timeNow = cell1.getDisplayValue();  
  
  cell2.setValue(timeNow);
  
  while (currentRow-6 <= rowCount){
    var cell = sheet.getRange("C"+currentRow);
    var idCell = sheet.getRange("B"+currentRow);
    var id = idCell.getDisplayValue();
    var test = sheet.getRange("G"+currentRow);
...
function getTime() {
  ...
  cell.setFormula('=SUBSTITUTE(SUBSTITUTE(RIGHT(NOW(),8)," ",0),":","")');
